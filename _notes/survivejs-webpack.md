...
```
Then, run 'npm run start' instead to start the dev server.
...
## Loading CSS
Loading CSS: css-loader and style-loader.
install.
```sh
...
Install the ExtractTextPlugin first:
```sh
...
```
* parts.loadCSS() has been moved from commonConfig to developmentConfig.
* parts.extractCSS is added into productionConfig
...
```
Update webpack.parts.js, create purifyCSS configurtion.
```javascript
